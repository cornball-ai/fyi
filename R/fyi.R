...
#'
#' Set `options(fyi.model = "Model Name")` to include the generating model
#' in the file header for provenance tracking.
#'
#' @param package Character. Package name.
...
use_fyi_md <- function(package, path = "fyi.md", src_dir = NULL,
  ...
  # Add metadata header
  model <- getOption("fyi.model", "")
  model_line <- if (nzchar(model)) paste0("<!-- Model: ", model, " -->\n") else ""
  header <- paste0(
...
use_fyi_docs <- function(package, dir = "man-md", pattern = NULL,
  ...
  for (i in seq_along(db)) {
    ...
    # Add header
    model <- getOption("fyi.model", "")
    model_line <- if (nzchar(model)) paste0(" | Model: ", model) else ""
    header <- paste0(
      "<!-- ", package, "::", topic_name, model_line, " -->\n",
